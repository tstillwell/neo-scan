function loadData() { // retrieve data from API and build table
		...
		$.ajax({
		  ...
		  for (var i = 0; i < number_of_neos; i++) { // Add a row to table for each NEO in response
			   ...
			   result_list_item += "<td><span class='miss-distance' data-distance-miles='" + Math.round(neo.close_approach_data[0].miss_distance.miles) + "'>" + Math.round(neo.close_approach_data[0].miss_distance[distance_unit]) + "</span></td>";
			   result_list_item += "</tr>";
